...
epsilon=0.3
theta=3
new_ratings_txt_path='./ml-100k/new_ratings'
random.seed(0)
time_div_path='./time_div.txt'
test_data_path='./test_data.txt'

class Old_Time_Genres(object):
                    ...
                    for i in range(19):
                        if (str(i) in user_genres.keys()):
                            ...
                            if(list_genres[i]=='1'):
                                user_genres[str(i)][0]=time
                                user_genres[str(i)][1]=time
            self.user_genres_freq[user]=self.calculate_feature_weight(user,user_genres)

    def calculate_feature_weight(self, user,user_genres):
        genre_freq = defaultdict(int)
        for genre, time_list in user_genres.items():
            Tnow=float(self.time_div[user])
            b=time_list[0]
            genre_freq[genre] = (time_list[0] - time_list[1]) / (Tnow - time_list[0]) * T_
    ...
    def als_algorithm(self):
        ...
        # temp=0
        # for user,rest in self.testset.items():
        #     for item,rating in rest.items():
        #         try:
        #             pred_rating=model.predict(int(user),int(item))
        #         except:
        #             print('wrong in user %d item %d'%(int(user),int(item)))
        #         temp+=float(pred_rating)**2-float(rating)**2
        # N=len(test_data_path)
        # RMSE=math.sqrt(temp/N)
        # print('RSME: '+str(RMSE))
        # # 按得分高低推荐前10电影
...
if __name__ == '__main__':
    ...
    filename_item_genres = os.path.join('ml-100k', 'u.item')
    old_model = Old_Time_Genres()
    old_model.generate_dataset(filename_UI, filename_item_genres)
    old_model.get_matrix()

    print(len(old_model.ui_rating))
    print('ui_rating= sum all gerens 0.2*time_weight[user][item]+0.3user_fenre_freq[user][genre]')
    print('ui_rating')
    print([str(key)+' '+str(item) for key,item in old_model.ui_rating.items()][0:5] )
    print('user genres')
    print([str(key)+' '+str(item)+'\n' for key,item in old_model.user_genres_freq.items()][0:5] )
    print('user item time_weicht')
    print([str(key)+' '+str(item)+'\n' for key,item in old_model.ui_time_weight.items()][0:5] )
